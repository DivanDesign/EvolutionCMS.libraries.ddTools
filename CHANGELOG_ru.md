...
## Версия 0.71 (2026-09-22)

* \* **Несовместимость.** `\ddTools::parseSource`: Разбирает произвольную строку, не подменяя `$modx->documentOutput` и не вызывая `OnParseDocument`:
	* \* Метод изначально был для кастомных строк, а не для тела документа, но раньше вложенный `$modx->parseDocumentSource` затирал вывод страницы. `$modx->parseDocumentSource` писал в `documentOutput` только чтобы плагины страницы могли переписать полный документ.
	* \* Те же шаги merge/eval и min/max parser passes, что у `$modx->parseDocumentSource`, теги в `$source` по-прежнему выполняются (настройки, поля, чанки, сниппеты, плейсхолдеры, URL).
* \* Внимание! Нарушена обратная совместимость.


## Версия 0.70 (2026-09-20)

* \+ `\DDTools\Tools\Objects::convertType` → Параметры → `$params->type` → Допустимые значения → `'stringGluedValues'`: Новое значение. Склеивает значения объекта/массива в одну строку (ключи отбрасываются, пустой вход даёт пустую строку).
* \+ README_ru.md: Русский перевод высокоуровневого README.
